...
signal_training_pth = '/users/cisnerosa/documents/wipac/scripts/ann/training_data/signal_sample/'
signal_training_data = loadFolder(signal_training_pth)

noise_training_pth = '/users/cisnerosa/documents/wipac/scripts/ann/training_data/noise_sample/'
noise_training_data = loadFolder(noise_training_pth)

signal_test_pth = '/users/cisnerosa/documents/wipac/scripts/ann/validation_data/signal_sample/'
signal_test_data = loadFolder(signal_test_pth)

noise_test_pth = '/users/cisnerosa/documents/wipac/scripts/ann/validation_data/noise_sample/'
...
trn_dataset = ClassificationDataSet(1024,nb_classes=2,class_labels=['Track','Other'])
...
#1024 inputs, hidden neurons, 1 output
net = buildNetwork(trn_dataset.indim, 16,64,16, trn_dataset.outdim,hiddenclass=TanhLayer,outclass=SoftmaxLayer)
#net = NetworkReader.readFrom('network_training_progress.xml')
...
while trainer.totalepochs < EPOCHS:
   ...
   print 'trn: {}%, test: {}%| epochs: {} | best: {}% accuracy (epoch {})'.format(str(100-trnresult)[:5],str(100-tstresult)[:5],trainer.totalepochs,best,best_epoch)
  
   train_err.append(trnresult)
   test_err.append(tstresult)
   
   if 100-int(tstresult) >= best:
       best = 100-int(tstresult)
       best_epoch = trainer.totalepochs
       NetworkWriter.writeToFile(net, 'network_training_progress.xml')
